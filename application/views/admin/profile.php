...
<body>
    ...
    <div class="p-4 sm:ml-64 bg-gray-50 min-h-screen font-popins">
        ...
        <div class="bg-white shadow-lg rounded p-5 mt-5 grid mx-auto grid-cols-1 md:grid-cols-2 gap-6">
            <?php foreach ($user as $row) : ?>
                ...
                <form action="<?= base_url('admin/aksi_update_profile') ?>" method="POST" enctype="multipart/form-data">
                    <input type="text" name="id_level" class="hidden" value="<?= $row->id_level ?>">
                    <div>
    ...
    <!-- SWEETALERT -->
    <?php if ($this->session->flashdata('successEditAdmin')) : ?>
        <script>
            Swal.fire({
                icon: 'success',
                title: 'Berhasil!',
                text: '<?= $this->session->flashdata('successEditAdmin') ?>',
    ...
    <?php if ($this->session->flashdata('errorNewPassAdmin')) : ?>
        <script>
            Swal.fire({
                icon: 'error',
                title: 'Gagal!',
                text: '<?= $this->session->flashdata('errorNewPassAdmin') ?>',
                showConfirmButton: false,
                timer: 1500,
            });
        </script>
    ...
    <?php if ($this->session->flashdata('errorConfirmPassAdmin')) : ?>
        <script>
            Swal.fire({
                icon: 'error',
                title: 'Gagal!',
                text: '<?= $this->session->flashdata('errorConfirmPassAdmin') ?>',
                showConfirmButton: false,
                timer: 1500,
            });
        </script>
    ...
    <?php if ($this->session->flashdata('errorEditAdmin')) : ?>
        <script>
            Swal.fire({
                icon: 'error',
                title: 'Gagal!',
                text: '<?= $this->session->flashdata('errorEditAdmin') ?>',
                showConfirmButton: false,
                timer: 1500,
            });
        </script>
